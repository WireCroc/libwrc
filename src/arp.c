...
void wc_arp_parse(const unsigned char *in, wc_arp *out) {
    ...
    if (arp->ar_hrd == ARPHRD_ETHER)
        ...
        out->hw_t = "OTHER";
    // protocol type
    out->p_t = wc_format("%d", arp->ar_pro);
    // hardware len
    ...
    // sender ip
    out->sender_ip = wc_format("%.2X.%.2X.%.2X.%.2X", arp->__ar_sip[0], arp->__ar_sip[1], arp->__ar_sip[2], arp->__ar_sip[3]);
    // target mac
    ...
    // target ip
    out->target_ip = wc_format("%.2X.%.2X.%.2X.%.2X", arp->__ar_tip[0], arp->__ar_tip[1], arp->__ar_tip[2], arp->__ar_tip[3]);
}
